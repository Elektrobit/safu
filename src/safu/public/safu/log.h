...
safuLogStatusE_t safuLogFuncV(safuLogLevelE_t level, const char *file,
                              const char *func, unsigned int line,
                              const char *fmt, va_list arg);
safuLogStatusE_t safuLogFuncF(safuLogLevelE_t level, const char *file,
                              const char *func, unsigned int line,
                              const char *fmt, ...)
    __attribute__((__format__(__printf__, 5, 6)));
safuLogStatusE_t safuLogFunc(safuLogLevelE_t level, const char *file,
                             const char *func, unsigned int line,
                             const char *fmt)
    __attribute__((__format__(__printf__, 5, 0)));

#define safuLogCrit(__message)                                                 \
  safuLogFunc(SAFU_LOG_LEVEL_CRIT, __FILE__, __func__, __LINE__, __message)
#define safuLogCritF(__message, ...)                                           \
  safuLogFuncF(SAFU_LOG_LEVEL_CRIT, __FILE__, __func__, __LINE__, __message,   \
               __VA_ARGS__)
#define safuLogErr(__message)                                                  \
  safuLogFunc(SAFU_LOG_LEVEL_ERR, __FILE__, __func__, __LINE__, __message)
#define safuLogErrF(__message, ...)                                            \
  safuLogFuncF(SAFU_LOG_LEVEL_ERR, __FILE__, __func__, __LINE__, __message,    \
               __VA_ARGS__)
#define safuLogErrErrno(__message)                                             \
  safuLogFuncF(SAFU_LOG_LEVEL_ERR, __FILE__, __func__, __LINE__,               \
               __message " - %s", strerror(errno))
#define safuLogWarn(__message)                                                 \
  safuLogFunc(SAFU_LOG_LEVEL_WARN, __FILE__, __func__, __LINE__, __message)
#define safuLogWarnF(__message, ...)                                           \
  safuLogFuncF(SAFU_LOG_LEVEL_WARN, __FILE__, __func__, __LINE__, __message,   \
               __VA_ARGS__)
#define safuLogInfo(__message)                                                 \
  safuLogFunc(SAFU_LOG_LEVEL_INFO, __FILE__, __func__, __LINE__, __message)
#define safuLogInfoF(__message, ...)                                           \
  safuLogFuncF(SAFU_LOG_LEVEL_INFO, __FILE__, __func__, __LINE__, __message,   \
               __VA_ARGS__)
#define safuLogDebug(__message)                                                \
  safuLogFunc(SAFU_LOG_LEVEL_DEBUG, __FILE__, __func__, __LINE__, __message)
#define safuLogDebugF(__message, ...)                                          \
  safuLogFuncF(SAFU_LOG_LEVEL_DEBUG, __FILE__, __func__, __LINE__, __message,  \
               __VA_ARGS__)
