...
#define FLAGS_CRIT                                                             \
  (SAFU_LOG_FLAG_WRITE_TO_STREAM | USE_STDERR_STREAM |                         \
   SAFU_LOG_FLAG_WITH_POSITION)
#define FLAGS_ERR                                                              \
  (SAFU_LOG_FLAG_WRITE_TO_STREAM | USE_STDERR_STREAM |                         \
   SAFU_LOG_FLAG_WITH_POSITION)
#define FLAGS_WARN                                                             \
...
#define FLAGS_DEBUG                                                            \
  (SAFU_LOG_FLAG_WRITE_TO_STREAM | USE_STDOUT_STREAM |                         \
   SAFU_LOG_FLAG_WITH_POSITION)
...
static safuContext_t safuContext = {
    .logLevelEntry =
        {
            [SAFU_LOG_LEVEL_NONE] =
                {.name = "NONE", .prefix = NULL, .stream = NULL, .flags = 0},
            [SAFU_LOG_LEVEL_CRIT] = {.name = "CRITICAL",
                                     .prefix = "CRITICAL: ",
                                     .stream = NULL,
                                     .flags = FLAGS_CRIT},
            [SAFU_LOG_LEVEL_ERR] = {.name = "ERROR",
                                    .prefix = "ERROR: ",
                                    .stream = NULL,
                                    .flags = FLAGS_ERR},
            [SAFU_LOG_LEVEL_WARN] = {.name = "WARNING",
                                     .prefix = "WARNING: ",
                                     .stream = NULL,
                                     .flags = FLAGS_WARN},
            [SAFU_LOG_LEVEL_INFO] = {.name = "INFO",
                                     .prefix = NULL,
                                     .stream = NULL,
                                     .flags = FLAGS_INFO},
            [SAFU_LOG_LEVEL_DEBUG] = {.name = "DEBUG",
                                      .prefix = "DEBUG: ",
                                      .stream = NULL,
                                      .flags = FLAGS_DEBUG},
        },
...
static safuLogStatusE_t _logFuncStream(safuLogMessage_t logMessage) {
  ...
  if ((status == SAFU_LOG_STATUS_SUCCESS) &&
      (safuContext.visibleLevel >= logMessage.level)) {
    if ((safuContext.filter[0] == '\0') || (_filterMatch(fileName) == true)) {
      ...
      if (entry->flags & SAFU_LOG_FLAG_WITH_POSITION) {
        retVal = fprintf(entry->stream, "%s%s%s - file '%s', line %d, %s()\n",
                         globalPrefix, levelPrefix, messageString, fileName,
                         logMessage.line, logMessage.func);
        if (retVal < 0) {
          status = SAFU_LOG_STATUS_ERROR;
        }
      } else {
...
safuLogLevelE_t safuLogLevelFromString(const char *logLevelStr) {
  ...
  if (logLevelStr != NULL) {
    ...
    for (logLevel = SAFU_LOG_LEVEL_NONE; logLevel < SAFU_LOG_LEVEL_COUNT;
         logLevel++) {
...
safuLogLevelE_t safuLogGetStreamLevel() { return safuContext.visibleLevel; }
...
safuLogStatusE_t _setFilterStr(const char *filterStr) {
  ...
  if (filterStr != NULL && filterLen < FILTER_MAX_SIZE) {
    char *retVal =
        strncpy(&safuContext.filter[0], filterStr, FILTER_MAX_SIZE - 1);
...
safuLogStatusE_t safuLogFuncV(safuLogLevelE_t level, const char *file,
  ...
  safuLogMessage_t logMessage = {
      .level = level, .file = file, .func = func, .line = line, .fmt = fmt};
  bool locked = false;

  va_copy(logMessage.ap, args);

  if ((level <= SAFU_LOG_LEVEL_NONE) || (level >= SAFU_LOG_LEVEL_COUNT) ||
      (fmt == NULL)) {
    status = SAFU_LOG_STATUS_ERROR;
