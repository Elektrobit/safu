...
ssize_t safuSendExactly(int fd, const void *buf, size_t len) {
  ...
  return safuTransferExactly(fd, (void *)buf, len, MSG_NOSIGNAL,
                             (safuTransferFunc_t *)send);
}
...
ssize_t safuReadFileToString(const char *fileName, ssize_t maxLength,
  ...
  if ((fileName == NULL) || (maxLength == 0) || (maxLength < -1) ||
      (string == NULL)) {
    safuLogErr("invalid parameter");
    result = -1;
  }
  ...
  if (size < 0) {
    ...
    safuLogErrF("file is bigger (%ld) than the specified maximum length (%ld)",
                size, maxLength);
    result = -1;
  }
  ...
  if (result == 0) {
    ...
    if (result < 0) {
      ...
      safuLogErrF("safuRecvExactly incomplete (%ld of %ld bytes)", result,
                  size);
      result = -1;
    } else {
      buffer[size] = '\0';
...
const char *safuGetHardwareId(void) {
  ...
  if (initialized == false) {
    ...
    if (bytes < minLength) {
      if (bytes < 0) {
        const char err[] = "safuReadFileToString failed, using default value "
                           "'%s' (file: '%s')";
        safuLogErrF(err, SAFU_HARDWAREID_DEFAULT_VALUE, hardwareIdFile);
      } else {
        const char err[] =
            "hardwareId too short (%ld), using default value '%s' (file: '%s')";
