...
safuResultE_t _replaceEntry(safuRingBuffer_t *ringBuffer, size_t idx,
                            void *element) {
    ...
    if (result == SAFU_RESULT_OK) {
      ...
      if (newData == NULL) {
        ...
        if (param->deleteEntries == true) {
          atomic_fetch_or(&ringBuffer->flags,
                          SAFU_RINGBUFFER_DELETE_ENTRIES_BIT);
        }
      }
...
safuResultE_t safuRingBufferDeleteMembers(safuRingBuffer_t *ringBuffer) {
  ...
  if (ringBuffer == NULL) {
    ...
    if (result == SAFU_RESULT_OK) {
        ...
        if (ringBuffer->data != NULL) {
          for (size_t i = 0; i < ringBuffer->elements; i += 1) {
            ...
            if (iterResult != SAFU_RESULT_OK) {
              safuLogWarnF(
                  "Delete for Ringbuffer idx:%lu failed (possible memory leak)",
                  i);
              result = iterResult;
            }
          }
      ...
      SAFU_PTHREAD_MUTEX_DESTROY(&ringBuffer->lock,
                                 result = SAFU_RESULT_FAILED);
    }
  }
