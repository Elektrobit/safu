...
#define _GET_ELEMENT_PTR(__vec, __idx)                                         \
  ((void *)&((uint8_t *)(__vec)->data)[(__vec)->elementSize * (__idx)])

// TODO: Improve allocating (and freeing?) memory during resize commands

static int safuVecCheck(const safuVec_t *vec) {
  int retval = 0;
  if ((vec == NULL) || (vec->data == NULL) || (vec->memorySize < 1) ||
      (vec->elementSize < 1)) {
    retval = -1;
  }
  return retval;
...
int safuVecFind(const safuVec_t *vec, uint32_t startIdx,
  ...
  if ((safuVecCheck(vec) < 0) ||
      ((vec->elementCount > 0) && (startIdx >= vec->elementCount)) ||
      (matchFunc == NULL)) {
    return -1;
  }
...
int safuVecFindGet(const safuVec_t *vec, void **element, uint32_t *idx,
                   safuVecFunc_t matchFunc, const void *matchData) {
  int result = -1;
...
int safuVecFindRemove(safuVec_t *vec, safuVecFunc_t matchFunc,
