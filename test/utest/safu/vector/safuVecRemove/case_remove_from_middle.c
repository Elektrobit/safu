...
void safuTestSafuVecRemoveFromMiddle(UNUSED void **state) {
...

  {
    ...
    safuTdata_t expectedTData[9] = {{1, "str 1"}, {2, "str 2"}, {3, "str 3"},
                                    {4, "str 4"}, {5, "str 5"}, {6, "str 6"},
                                    {7, "str 7"}, {8, "str 8"}, {9, "str 9"}};
    for (size_t i = 0; i < fillSize; i++) {
      assert_true(
          safuMatchSafuTdata((const void *)&((safuTdata_t *)vec.data)[i],
                             (const void *)&expectedTData[i]));
    }
  }

  {
    ...
    assert_int_equal(0, retval);
    assert_int_equal(fillSize, vec.elementCount);
    safuTdata_t expectedTData[8] = {{1, "str 1"}, {2, "str 2"}, {4, "str 4"},
                                    {5, "str 5"}, {6, "str 6"}, {7, "str 7"},
                                    {8, "str 8"}, {9, "str 9"}};
    for (size_t i = 0; i < fillSize; i++) {
      assert_true(
          safuMatchSafuTdata((const void *)&((safuTdata_t *)vec.data)[i],
...

  {
    ...
    assert_int_equal(0, retval);
    assert_int_equal(fillSize, vec.elementCount);
    safuTdata_t expectedTData[7] = {{1, "str 1"}, {2, "str 2"}, {4, "str 4"},
                                    {6, "str 6"}, {7, "str 7"}, {8, "str 8"},
                                    {9, "str 9"}};
    for (size_t i = 0; i < fillSize; i++) {
      assert_true(
          safuMatchSafuTdata((const void *)&((safuTdata_t *)vec.data)[i],
