...
void safuTestSafuVecRemoveLastElement(UNUSED void **state) {
  ...
  const safuTdata_t expectedTData[5] = {
      {0, "str 0"}, {1, "str 1"}, {2, "str 2"}, {3, "str 3"}, {4, "str 4"}};

  int retval = safuVecRemove(&vec, 4);
  assert_int_equal(0, retval);
  assert_int_equal(4, vec.elementCount);
  for (size_t i = 0; i < 4; i++) {
    assert_true(safuMatchSafuTdata((void *)&((safuTdata_t *)vec.data)[i],
                                   (void *)&expectedTData[i]));
  }
