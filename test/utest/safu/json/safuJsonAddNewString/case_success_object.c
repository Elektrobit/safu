...
SETUP(safuTestSafuJsonAddNewStringSuccessObject)
TEARDOWN(safuTestSafuJsonAddNewStringSuccessObject)
...
static void _testCase(struct json_object *jobj, char *name, char *val) {
  ...
  assert_string_equal(
      json_object_get_string(json_object_object_get(jobj, name)), val);
}

void safuTestSafuJsonAddNewStringSuccessObject(void **state) {
  TEST("safuJsonAddNewString");
  SHOULD("%s", "successfully add a string to a JSON object");
