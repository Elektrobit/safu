...
static void _testCase(struct json_object *jarray, const char *key,
                      struct json_object *jadd) {
  PARAM("key = \"%s\"", key);
...
void safuTestSafuJsonAddObjectErrStringIndexArray(void **state) {
  ...
  SHOULD("%s",
         "fail to add a JSON object to a JSON array with a sting as index key");
