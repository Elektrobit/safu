...
int safuTestSafuJsonAddNewTimestampErrAddArrayObjectSetup(UNUSED void **state) {
  return 0;
}
int safuTestSafuJsonAddNewTimestampErrAddArrayObjectTeardown(
    UNUSED void **state) {
  return 0;
}

static void _testCase(const char *key, struct timespec timestamp) {
  PARAM("key = \"%s\"", key);
  PARAM("timestamp = { tv_sec = %ld, tv_nsec = %ld }", timestamp.tv_sec,
        timestamp.tv_nsec);
  ...
  will_return(__wrap_json_object_array_add,
              __real_json_object_array_add(jtimewrap, jtvSec));
  ...
  will_return(__wrap_json_object_array_add,
              __real_json_object_array_add(jtimewrap, jtvNsec));

  struct json_object *jtimestamp =
      safuJsonAddNewTimestamp(NULL, key, &timestamp);
...
void safuTestSafuJsonAddNewTimestampErrAddArrayObject(UNUSED void **state) {
  TEST("safuJsonAddNewTimestamp");
  SHOULD("%s", "fail to add an array for a timestamp to a json object");
  ...
  for (size_t i = 0; i < ARRAY_SIZE(testRows); i++) {
    _testCase(testRows[i].key, testRows[i].time);
  }
}
