...
static void _testCase(struct json_object *jobj, size_t idx,
  ...
  will_return(__wrap_json_object_array_add,
              __real_json_object_array_add(jobj, jtimewrap));

  struct json_object *jtimestamp =
      safuJsonAddNewTimestamp(jobj, NULL, &timestamp);

  assert_non_null(jtimestamp);
  assert_int_equal(
      json_object_get_int64(json_object_array_get_idx(jtimestamp, 0)),
      timestamp.tv_sec);
  assert_int_equal(
      json_object_get_int64(json_object_array_get_idx(jtimestamp, 1)),
      timestamp.tv_nsec);
