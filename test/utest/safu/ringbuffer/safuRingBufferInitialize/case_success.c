...
void safuTestSafuRingBufferInitializeSuccess(void **state) {
  safuUnitTestState_t *test = *(safuUnitTestState_t **)state;
  safuRingBufferParam_t paramA = {
      .elements = _TEST_ELEMENTS, .deleteEntries = false, .deleteFunc = NULL};
  safuRingBufferParam_t paramB = {.elements = _TEST_ELEMENTS,
                                  .deleteEntries = true,
                                  .deleteFunc = _dummyDeleteFunc};
  ...
  assert_int_equal(SAFU_FLAG_HAS_INITIALIZED_BIT(&test->ringBuffer.flags),
                   true);
  assert_int_equal(SAFU_FLAG_HAS_DELETE_ENTRIES_BIT(&test->ringBuffer.flags),
                   false);
  assert_int_equal(test->ringBuffer.memorySize,
                   (_TEST_ELEMENTS * SAFU_RINGBUFFER_ELEMENT_SIZE));
