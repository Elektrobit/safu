...
void safuTestSafuTransferExactlyErrSendBytesLtZero(UNUSED void **state) {
  ...
  SHOULD("%s", "return transfered bytes if the transfer function returns a "
  ...
  result = safuTransferExactly(fileDescriptor, buffer, length, MSG_NOSIGNAL,
                               safuMockTransferFunc);
